...
function initSocket(server) {
    ...
    async function removeParticipantFromCall(teamId, socketId) {
        ...
        if (call.socketToUser.size < 2) {
            ...
            try {
                const msgText = duration > 0
                    ...
                    : `Call missed`;
                const isValidTeamId = mongoose_1.default.Types.ObjectId.isValid(call.teamId);
                const isValidSenderId = mongoose_1.default.Types.ObjectId.isValid(call.callerUserId);
                if (mongoose_1.default.connection.readyState === 1 && isValidTeamId && isValidSenderId) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join_active_call', ({ teamId }) => {
            const call = activeCalls.get(teamId);
            if (!call) {
