...
async function seed() {
  try {
    ...
    const t1Milestone1 = new Milestone({
      teamId: team1._id,
    ...
    const t1Milestone2 = new Milestone({
      teamId: team1._id,
    ...
    const t2Milestone1 = new Milestone({
      teamId: team2._id,
    ...
    for (const t of tasksT1) {
      await new Task({ ...t, teamId: team1._id }).save();
    }
    for (const t of tasksT2) {
      await new Task({ ...t, teamId: team2._id }).save();
    }
